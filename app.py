...
def get_weather(location: str):
    ...
    except requests.exceptions.HTTPError as e: status=e.response.status_code; detail=f"HTTP {status}"; msg=""; try: msg=e.response.json().get('error',{}).get('message',''); detail+=f": {msg}" if msg else "" except: pass; logging.error(f"HTTP error weather {location}: {detail}");
...
def perform_web_search(query: str, num_results: int = 3): # Reduced default results
...
def call_gemini(prompt: str, is_json_output: bool = False):
    ...
    try:
        cfg=genai.types.GenerationConfig(temperature=0.6, response_mime_type=mime) # Slightly lower temp
...
def ask_assistant():
    ...
    try:
        ...
        if WEATHER_API_KEY:
            prompt=f"""Analyze query: "{question}". Is it weather/forecast? If yes, identify location. ONLY JSON: {{"is_weather_query": boolean, "location": string_or_null}}."""
            raw, err=call_gemini(prompt, is_json_output=True)
            if not err:
                try: clean=raw.strip(); # ... (JSON cleaning) ...
                ...
                except Exception as e: logging.exception(f"Weather intent JSON error: {e}. Raw: {raw}"); details.update({"intent_ok":False, "err":f"Weather JSON parse err: {type(e).__name__}"})
            else: logging.error(f"Weather intent fail: {err}"); details.update({"intent_ok":False, "err":f"Intent fail: {err}"})

        # 1b. Check Routing Intent (If not weather)
        if not is_weather:
            prompt=f"""Analyze query: "{question}". Is it asking for route/directions between two locations? If yes, identify Origin & Destination. ONLY JSON: {{"is_routing_query": boolean, "origin": string_or_null, "destination": string_or_null}}."""
            raw, err=call_gemini(prompt, is_json_output=True)
            if not err:
                try: clean=raw.strip(); # ... (JSON cleaning) ...
                    if clean.startswith("```json"): clean=clean[7:-3].strip() elif clean.startswith("```"): clean=clean[3:-3].strip()
                    d=json.loads(clean); is_routing=d.get("is_routing_query") is True; route_origin=d.get("origin"); route_dest=d.get("destination")
                ...
                except Exception as e: logging.exception(f"Routing intent JSON error: {e}. Raw: {raw}"); details.update({"route_intent":False, "err":f"Routing JSON parse err: {type(e).__name__}"})
        ...
        if is_weather and weather_loc and WEATHER_API_KEY:
             ...
             if w_err: details.update({"weather_ok": False, "err": w_err}); logging.error(f"WeatherAPI error: {w_err}"); prompt = f"Friday: Inform user politely of weather lookup issue for '{weather_loc}'. Problem: '{w_err}'. Suggest check location/try later."; resp, _ = call_gemini(prompt); final_text = resp or f"Sorry, couldn't get weather for '{weather_loc}': {w_err}"; details["final_src"] = "weather_api_err_ai"
                 ...
                 try: # Safely extract data and build summaries/viz
                     ...
                     if lat is not None and lon is not None: map_data={"type":"point", "latitude":lat, "longitude":lon, "zoom":11, "marker_title":full}; logging.info(f"Prep map data: {lat},{lon}") # Set type to 'point'
        ...
        # 2b. Handle Routing
            ...
            if origin_err or dest_err:
                 ...
                 map_data = {"type": "route", "origin": {"name": route_origin, "coords": list(origin_coords)}, "destination": {"name": route_dest, "coords": list(dest_coords)}} # [lat, lon] format
                 logging.info("Prepared map data for routing points.")
                 prompt = f"You are Friday. User asked for route: {route_origin} -> {route_dest}. You will show a map. Provide a brief intro text like 'Okay, showing the map for the route from {route_origin} to {route_dest}.'"
                 final_text, _ = call_gemini(prompt); final_text = final_text or f"Okay, here's the map for {route_origin} and {route_dest}."; details["final_src"] = "routing_map_intro_ai"
        ...
        if final_text is None:
            ...
            if needed and query:
                ...
                s_res, s_err = perform_web_search(query, num_results=3) # Use fewer results for search
                if s_err: details.update({"search_ok":False, "err":s_err}); logging.error(f"Search func error: {s_err}"); prompt=f"Friday: Inform user politely of technical problem searching web regarding '{query}'. Internal error: '{s_err}'. Apologize."; resp,_=call_gemini(prompt); final_text=resp or f"Sorry, tech issue searching: {s_err}"; details["final_src"]="search_func_err_ai"
                    ...
                    prompt=f"""You are Friday, an AI assistant focused on providing clear and relevant answers. User asked: "{question}". Web search for "{query}" findings:\n---BEGIN RESULTS---\n{s_res if s_res else "No specific results found."}\n---END RESULTS---\nAnswer user's question based *strictly* on results. If results have multiple items & user asked for singular, try identify most prominent based *only* on snippets (time/emphasis etc) & state interpretation. If ambiguous/multiple seem equal, list key items briefly. If no relevant results, state that clearly & maybe answer generally if appropriate (distinguish clearly). Be concise, only the answer. Answer:"""
                    resp, err=call_gemini(prompt)
                    if err: logging.error(f"AI search synthesis fail: {err}"); final_text=f"Looked online for '{query}' but trouble summarizing."; details.update({"err":err, "final_src":"search_synth_err"})
...
if __name__ == '__main__':
    ...
    try: app.run(host='0.0.0.0', port=5000, debug=is_debug, ssl_context=ssl_ctx, threaded=True) # Enable threading for potentially better handling of concurrent requests
