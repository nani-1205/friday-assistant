...
def get_weather(location: str):
    ...
    try:
        ...
        status_code = http_err.response.status_code; error_detail = f"HTTP error {status_code}"
...
def perform_web_search(query: str, num_results: int = 5):
    ...
    try:
        ...
        for result in search_results:
            ...
            max_snippet_len = 250; processed_results.append(f"Title: {title}\nLink: {link}\nSnippet: {snippet[:max_snippet_len]}...")
    ...
    except Exception as e: logging.exception(f"Unexpected error during DDG web search for '{query}': {e}"); return None, f"An unexpected error occurred during the web search ({type(e).__name__}). Please try again."
...
def call_gemini(prompt: str, is_json_output: bool = False):
    ...
    try:
        ...
        else: logging.error(f"Gemini returned empty/unexpected response: {response}"); return None, "The AI returned an empty or unexpected response."
    except Exception as e:
...
def ask_assistant():
    ...
    if not model: logging.error("Received /ask request but AI model is not available."); return jsonify({"error": "AI Model is not initialized. Cannot process request."}), 500
    try:
        data = request.get_json()
        if not data or not isinstance(data, dict): logging.warning(f"Invalid request format to /ask from {request.remote_addr}: Not JSON object."); return jsonify({"error": "Invalid request format."}), 400
        question = data.get('question', '').strip()
        if not question: logging.warning(f"Empty question received in /ask request from {request.remote_addr}."); return jsonify({"error": "Question cannot be empty."}), 400
        ...
        else: interaction_details["intent_analysis_success"] = None # Skipped
        ...
        if mongodb_ready and collection is not None:
            interaction_data = {"timestamp": datetime.utcnow(), "request_ip": request.remote_addr, "question": question, "response": final_response_text, "model_used": GEMINI_MODEL_NAME, "processing_time_seconds": round(processing_time, 2), "details": interaction_details}
    ...
    except Exception as e: logging.exception("Critical unexpected error occurred in /ask endpoint."); return jsonify({"error": "A critical internal server error occurred. Please check server logs."}), 500
...
if __name__ == '__main__':
    ...
    else: logging.warning(f"Certificates ('{cert_file}', '{key_file}') not found. Starting {server_type} server. Microphone may fail on non-localhost addresses.")
