...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (supportsRecognition) {
     ...
     function createNotification(title, message, type = 'info') {
        ...
        const notification = document.createElement('div'); notification.className = `notification ${type}`; // Base + type class
    ...
    function clearAssistantOutputArea() {
        console.log("[DEBUG] Clearing assistant output area..."); // Log clearing
    ...
    function createMapVisualization(mapVizData, containerElement) {
         ...
         } catch (error) { console.error("[DEBUG] Error setting up map container:", error); if(containerElement) containerElement.innerHTML = "<span>[Map Setup Error]</span>"; else { /* Can't display error in container if it failed */ } }
    ...
    function hideLoadingIndicator() {
        ...
        if (!assistantSpeaking && !isListening && !statusTextElement?.dataset.error) {
            updateStatus('SYSTEMS ONLINE'); // Reset to default online status
        }
    ...
    async function sendMessage() {
        ...
        // displayContent({ userQuery: question }); // Removed brief query display
        ...
        try {
            ...
            try { data = await response.json(); console.log("[DEBUG] Received data from backend:", data); }
            catch (jsonError){ console.error("[DEBUG] JSON Parse Error:", jsonError); data = { error: `Invalid response from server (Status: ${response.status})` };} // Create error object
    ...
    function speakResponse(textToSpeak) {
         if (!supportsSynthesis || !synth || !textToSpeak || typeof textToSpeak !== 'string' || textToSpeak.trim() === '') { console.log("[DEBUG] Speech skipped."); if(assistantSpeaking){ assistantSpeaking=false; /* reset state if needed */ } return; }
         if (synth.speaking || synth.pending) { console.log("[DEBUG] Cancelling previous speech."); synth.cancel(); if(assistantSpeaking){ assistantSpeaking=false; /* reset state if needed */ } }
         ...
         utterance.onstart = () => { console.log("[DEBUG] Speech started (onstart event)."); assistantSpeaking = true; updateStatus('Speaking...'); if (visualization) visualization.style.animationPlayState = 'running'; if (listenButton) listenButton.disabled = true; };
         utterance.onend = () => { console.log("[DEBUG] Speech finished (onend event)."); assistantSpeaking = false; hideLoadingIndicator(); /* Hide loading *after* speech finishes */ if (!isListening && !statusTextElement?.dataset.error) { updateStatus('SYSTEMS ONLINE'); if (visualization) visualization.style.animationPlayState = 'paused'; } if (listenButton) listenButton.disabled = !supportsRecognition; if(userInput) try{userInput.focus();}catch(e){} };
         utterance.onerror = (event) => { console.error('[DEBUG] Speech error (onerror event):', event.error, event); assistantSpeaking = false; hideLoadingIndicator(); /* Hide loading on error too */ createNotification("Speech Error", `Speech playback failed: ${event.error}`, "error"); if (!isListening) { updateStatus('Speech Error', true); if (visualization) visualization.style.animationPlayState = 'paused'; } if (listenButton) listenButton.disabled = !supportsRecognition; };
         setTimeout(() => {
             try { if (synth) { console.log("[DEBUG] Attempting synth.speak..."); synth.speak(utterance); console.log(`[DEBUG] synth.speak called. Status now: speaking=${synth.speaking}, pending=${synth.pending}`); } else { console.error("[DEBUG] Synth object unavailable before speak."); createNotification("Speech Error", "Synthesis engine unavailable.", "error"); } }
             catch (speakError) { console.error("[DEBUG] Error occurred during synth.speak() call:", speakError); createNotification("Speech Error", `Playback error: ${speakError.message}`, "error"); assistantSpeaking = false; hideLoadingIndicator(); }
         }, 100);
    } // End speakResponse
